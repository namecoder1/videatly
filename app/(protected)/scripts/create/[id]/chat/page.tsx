...
import { User, CirclePause, ArrowLeft, Loader2, Save, ArrowRight, Brain } from 'lucide-react'
  ...
  const handleSave = async () => {
	...
	return (
        ...
        <Card className='p-4 mb-6'>
          <h3 className='font-semibold mb-2'>Script Details:</h3>
          <div className='space-y-1 text-sm'>
            <p><span className='font-medium'>Tone:</span> {scriptData?.tone}</p>
            <p><span className='font-medium'>Verbosity:</span> {scriptData?.verbosity}</p>
            <p><span className='font-medium'>Target Audience:</span> {scriptData?.target_audience}</p>
            <p><span className='font-medium'>Script Type:</span> {scriptData?.script_type}</p>
            <p><span className='font-medium'>Duration:</span> {scriptData?.duration}</p>
            <p><span className='font-medium'>Persona:</span> {scriptData?.persona}</p>
            <p><span className='font-medium'>Structure:</span> {scriptData?.structure}</p>
          </div>
        </Card>
        ...
        <form onSubmit={handleSubmit} className="fixed bottom-2 w-full max-w-sm mb-8 rounded-3xl backdrop-blur">
				{isLoading && (
					<button
						onClick={stop}
