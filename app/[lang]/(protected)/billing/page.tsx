...
import { Clock, Loader2, OctagonX, Settings2, ShoppingBag, Wallet } from 'lucide-react'
    ...
    <section className="space-y-6">
      ...
      <div className='flex flex-col'>
        <div className='flex items-center gap-3'>
          ...
          <h1 className='text-lg sm:text-2xl md:text-3xl font-bold tracking-tight mr-16'>Billing</h1>
        </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <p className='text-sm text-muted-foreground'>
                          {userData?.subscription_renewal === false ? (
                              <>Ends on {userData?.subscription_end ? new Date(userData.subscription_end).toLocaleDateString() : 'N/A'}</>
                            ) : (
                              <>Ends in {userData?.subscription_end ?
                      ...
                      <p className="text-sm text-muted-foreground">
                        {userData?.subscription_renewal === false ? (
                          ...
                          userData?.pending_subscription && userData?.pending_subscription !== userData?.subscription
                            ? <>The plan will be automatically renewed on <span className="font-medium">{userData?.subscription_end ? new Date(userData.subscription_end).toLocaleDateString() : 'N/A'}</span>. You will be <span className="text-yellow-800 font-medium">downgraded to the {getPlanLabel(userData.pending_subscription)} plan</span> at the end of the current period.</>
                            : <>The plan will be automatically renewed on <span className="font-medium">{userData?.subscription_end ? new Date(userData.subscription_end).toLocaleDateString() : 'N/A'}</span>.</>
                        )}
                      </p>
                      <Progress 
              ...
              <Card className="overflow-hidden border-2 border-border">
                <CardHeader className="p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-base font-medium">No Active Subscription</h3>
                      <p className="text-sm text-muted-foreground">Upgrade to a paid plan to access all features</p>
                    </div>
                  </div>
                </CardHeader>
              </Card>
      ...
      <div className="space-y-4 pt-6">
        <div className="flex flex-col items-start justify-between">
          ...
          <p className='text-gray-600 text-sm mt-1'>View your recent payments and subscriptions</p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="px-4 py-3 text-left font-medium">Invoice Url</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {payments.slice(0, 5).map((payment) => (
                    <tr key={payment.id} className="hover:bg-muted/50 transition-colors">
                      <td className="px-4 py-3 font-mono">{payment.currency === 'eur' ? '€' : ''}{(payment.amount / 100).toFixed(2)}</td>
                      <td className="px-4 py-3 capitalize">{payment.product === 'ideas' ? 'Idea Tokens' : payment.product === 'scripts' ? 'Script Tokens' : `${payment.product} Plan`}</td>
                      ...
                      <td className="px-4 py-3 text-muted-foreground hidden sm:table-cell">{new Date(payment.created_at).toLocaleDateString()}</td>
                      <td className={`px-4 py-3 text-muted-foreground hidden sm:table-cell `}>
                        {payment.metadata.type?.charAt(0).toUpperCase() + payment.metadata.type?.slice(1).toLowerCase()}
                      </td>
                      {payment.metadata.hosted_invoice_url ? (
                        <td className="px-4 py-3 text-muted-foreground hidden sm:table-cell">
                          <Link href={payment.metadata.hosted_invoice_url || ''} className='hover:underline underline-offset-2'>
                            View invoice
                          </Link>
                        </td>
                      ) : (
