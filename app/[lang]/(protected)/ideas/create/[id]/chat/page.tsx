...
const IdeaChatPage = ({ params }: { params: { id: string } }) => {
    ...
    initialMessages: [
      {
        ...
        role: "assistant",
        content:
          ideaData && profile?.spoken_language === "it"
            ...
            : profile?.spoken_language === "en"
              ? `Hello, ${profile?.name}. I've seen that you want to create a video with these characteristics.
					Do you already have an idea in mind or do you want me to help you develop an idea?`
              : profile?.spoken_language === "es"
                ? `Hola, ${profile?.name}. He visto que quieres crear un video con estas características.
					¿Ya tienes una idea o quieres que te ayude a desarrollar una idea?`
  ...
  const handleSaveIdea = async () => {
    ...
    setIsSaving(true);
    try {
      const result = await saveIdea(id, ideaData, messages);
    ...
    .filter(
      (m) =>
        ...
        ((m.content.includes("**📝 Title**") &&
          m.content.includes("**📋 Description**")) || // English
          (m.content.includes("**📝 Titolo**") &&
            m.content.includes("**📋 Descrizione**")) || // Italian
          (m.content.includes("**📝 Título**") &&
            m.content.includes("**📋 Descripción**")) || // Spanish
          (m.content.includes("**📝 Titre**") &&
            m.content.includes("**📋 Description**"))) // French
    )
    .map((m) => m.content);
...
const IdeaInfo = ({
  ...
  const { description, video_style, video_type, video_length, video_target } =
    idea;
      ...
      <CardHeader className="space-y-4">
        ...
        <CardDescription className="text-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <IdeaInfoItem
              label={
                dict?.ideaCreatePage?.form?.targetInterest || "Video Target"
              }
  ...
  return (
    <div className="rounded-3xl border bg-gradient-to-tr from-zinc-50 to-zinc-100 px-6 py-4 dark:bg-zinc-900 shadow">
      <ReactMarkdown
        ...
        components={{
          strong: ({ node, ...props }) => (
            <strong className="font-bold text-primary space-x-2" {...props} />
          ),
          h4: ({ node, ...props }) => (
            <h4 className="font-bold text-primary mb-1 mt-1" {...props} />
          ),
          p: ({ node, ...props }) => (
            <p
              className="last:mb-0 [&:not(:has(+ol))]:mb-6 mt-1  text-gray-700 dark:text-gray-200"
              {...props}
            />
          ),
          ol: ({ node, ...props }) => (
            <ol
              className="space-y-1.5 list-decimal list-inside pb-6"
              {...props}
            />
          ), // togli bullet e padding
          li: ({ node, ...props }) => (
            <li
              className="mb-0.5 mt-0.5 text-gray-700 dark:text-gray-200"
              {...props}
            />
          ),
        }}
