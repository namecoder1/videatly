...
export async function POST(req: NextRequest) {
  ...
  async function processPayment(user_id: string, tokens: string, tool: string, session: Stripe.Checkout.Session) {
    ...
    if (tokensError && tokensError.code !== 'PGRST116') { // PGRST116 is "no rows returned"
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (!session.metadata?.tokens || !session.metadata?.tool || !session.metadata?.user_id) {
