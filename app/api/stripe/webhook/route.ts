...
if (process.env.NODE_ENV === "development") {
  ...
  baseUrl = ensureUrlScheme(process.env.NEXT_PUBLIC_BASE_URL_TEST!);
} else {
  endpointSecret = process.env.STRIPE_WEBHOOK_SECRET!;
  baseUrl = ensureUrlScheme(process.env.NEXT_PUBLIC_BASE_URL!);
}
...
export async function POST(req: NextRequest) {
  ...
  if (
    ...
    (event.type === "invoice.paid" &&
      typeof (event.data.object as any).subscription === "string")
  ) {
