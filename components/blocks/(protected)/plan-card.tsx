...
const PlanCard = ({
  ...
  const getButtonText = () => {
    ...
    if (
      ...
      userData &&
      (userData.subscription === "pro" || userData.subscription === "ultra")
    )
      return "Downgrade";
    if (userData?.pending_subscription?.toLowerCase() === name.toLowerCase())
      return "Active Next Month";
    return action;
  };
  ...
  const isPendingSubscription =
    userData?.pending_subscription?.toLowerCase() === name.toLowerCase();
    ...
    >
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex flex-col items-start justify-between">
            <h3 className="text-xl font-semibold flex justify-between items-center w-full">
              {name}{" "}
              {!isRenewed && name === "Free" && (
                <span className="text-xs text-muted-foreground">Next Plan</span>
              )}
            </h3>
        ...
        <Button
          type="button"
          variant={
            isCurrentPlan ? "outline" : isUpgrade ? "default" : "secondary"
          }
          ...
          onClick={handleClick}
          disabled={
            loading || isCurrentPlan || isPendingSubscription || disabled
          }
        >
          {loading ? (
            ...
            <div className="flex items-center justify-center gap-2">
              {getButtonText()}
              {isUpgrade && !isPendingSubscription && (
