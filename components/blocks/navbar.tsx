		...
		<nav className=' md:mx-4 sticky top-4 z-50'>
			<div className='mx-4 my-4 flex items-center justify-between max-w-3xl md:mx-auto border border-border py-2.5 px-4 rounded-full bg-white/95 shadow-sm hover:shadow-md transition-all duration-300'>
				...
				<ul className='hidden md:flex items-center gap-6'>	
					<li>
						<CustomLink onClick={(e) => handleScrollToElement(e, 'use-cases')} href="#use-cases" className='text-sm flex items-center gap-2 hover-underline-animation text-muted-foreground hover:text-foreground transition-all duration-200'>
							<ListVideo className='size-4' /> {dict.navbar.useCases}
						</CustomLink>
					</li>
					<li>
						<CustomLink onClick={(e) => handleScrollToElement(e, 'features')} href="#features" className='text-sm flex items-center gap-2 hover-underline-animation text-muted-foreground hover:text-foreground transition-all duration-200'>
							<Blocks className='size-4' /> {dict.navbar.features}
						</CustomLink>
					</li>
					<li>
						<CustomLink onClick={(e) => handleScrollToElement(e, 'how-it-works')} href="#how-it-works" className='text-sm flex items-center gap-2 hover-underline-animation text-muted-foreground hover:text-foreground transition-all duration-200'>
						...
						<DropdownMenu open={isMenuOpen} onOpenChange={setIsMenuOpen}>
							...
							<DropdownMenuContent className='w-56 mr-4 mt-3 p-2 rounded-3xl border-border/50 shadow-lg animate-in slide-in-from-top-2 duration-200'>
								<DropdownMenuItem className='flex items-center gap-2 py-2.5 px-3 rounded-2xl hover:bg-muted/50 transition-colors'>
									<CustomLink onClick={(e) => handleScrollToElement(e, 'use-cases')} href="#use-cases" className='text-sm flex items-center gap-2 w-full'>
										<ListVideo className='size-4' /> {dict.navbar.useCases}
									</CustomLink>						
								</DropdownMenuItem>
								<DropdownMenuItem className='flex items-center gap-2 py-2.5 px-3 rounded-2xl hover:bg-muted/50 transition-colors'>
									<CustomLink onClick={(e) => handleScrollToElement(e, 'features')} href="#features" className='text-sm flex items-center gap-2 w-full'>
										<Blocks className='size-4' /> {dict.navbar.features}
									</CustomLink>								
								</DropdownMenuItem>
								<DropdownMenuItem className='flex items-center gap-2 py-2.5 px-3 rounded-2xl hover:bg-muted/50 transition-colors'>
									<CustomLink onClick={(e) => handleScrollToElement(e, 'how-it-works')} href="#how-it-works" className='text-sm flex items-center gap-2 w-full'>
										<CircleHelp className='size-4' /> {dict.navbar.howItWorks}
									</CustomLink>							
								</DropdownMenuItem>
							</DropdownMenuContent>
